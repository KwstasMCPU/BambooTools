...
def test_completeness(animals_dataset):
    ...
    assert (
        result["complete values"].max() <= animals_dataset.shape[0]
    ), "Max value of `complete values` cannot exceed total records."
    assert (
        result["completeness ratio"].max() <= 1.0
...
def test_completeness_per_group(animals_dataset):
    ...
    assert (
        result["weight"].loc["cat", "total"] == n_cats
...
def test_outlier_summary(penguins_dataset):
    ...
    assert (
        n_penguins_grp["Female", "Adelie"]
        == result["total_records"].loc[("Female", "Adelie"), "bill_depth_mm"]
...
def test_duplication_summary(penguins_dataset):
    ...
    assert (
        result.loc["total duplicated records"].item() == total_duplicates
